#!/usr/bin/env python3
"""
Send-to-MASSO Manager - HMI-style Tkinter uploader (v1.8.2)
...
APP_NAME = "Send-to-MASSO Manager v1.8.2"
...
class SendGui:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(top, text="v1.8.2  •  queue + QR + Image branding", style="Version.TLabel").pack(side="right", pady=(9, 0))
        ...
        # Larger HMI values
        big_values = ttk.Frame(status, style="Panel.TFrame")
        big_values.grid(row=3, column=0, sticky="ew", pady=(0, 10))
        big_values.columnconfigure(0, weight=1)
        big_values.columnconfigure(1, weight=1)
        ...
        self._detail_row(status, 5, "Last Packet", self.last_seen_var)
        self._detail_row(status, 6, "Raw", self.raw_var)

        logo_frame = ttk.Frame(status, style="SubPanel.TFrame", padding=8)
        logo_frame.grid(row=7, column=0, sticky="ew", pady=(10, 0))
        ...
        ttk.Button(queue_btns, text="QR Queue...", command=self.generate_qr_for_queue).pack(side="left", padx=(0, 6))
        ...
        self.log("V1.8.1 HMI GUI loaded. Batch queue + QR-code build.")
    ...
    def _load_brand_logo(self) -> None:
        ...
        try:
            ...
            self.logo_label.configure(text="TANN Corp.")
    ...
    def _big_status_box(self, parent: tk.Widget, column: int, label: str, var: tk.StringVar) -> None:
        box = ttk.Frame(parent, style="SubPanel.TFrame", padding=10)
        box.grid(row=0, column=column, sticky="ew", padx=(0, 6) if column == 0 else (6, 0))
        ttk.Label(box, text=label, style="Muted.TLabel", background=self.PANEL_2).pack(anchor="w")
        tk.Label(
            box,
            textvariable=var,
            bg=self.PANEL_2,
