#!/usr/bin/env python3
"""
TURBO Send-to-MASSO Manager - HMI-style Tkinter proof of concept (v1.7)
...
APP_NAME = "TURBO Send-to-MASSO Manager v1.7.10"
            ...
            for variant_name, start_packet in start_packets:
                ...
                for attempt in range(1, 4):
                    ...
                    if code == b"\x00\x00":
                        ...
                        self.log(f"Start upload accepted via {variant_name}; ACK={start_ack.hex(' ')}")
                        break
...
class TurboSendGui:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(top, text="TURBO Send-to-MASSO Manager", style="Header.TLabel").pack(side="left")
        ttk.Label(top, text="v1.7.10  •  refresh TX socket before each upload", style="Version.TLabel").pack(side="right", pady=(9, 0))
        ...
        ttk.Label(send, text="example: \\Test\\", style="Muted.TLabel").grid(row=3, column=2, sticky="w", pady=(2, 8))
        ...
        self.send_btn.grid(row=4, column=0, sticky="ew", pady=(8, 0))
        self.upload_progress = ttk.Progressbar(send, maximum=100, mode="determinate")
        self.upload_progress.grid(row=4, column=1, columnspan=2, sticky="ew", padx=(8, 0), pady=(8, 0))

        self.last_sent_var = tk.StringVar(value="No successful upload yet")
        last = ttk.Frame(send, style="SubPanel.TFrame", padding=10)
        last.grid(row=5, column=0, columnspan=3, sticky="ew", pady=(12, 0))
        ...
        self.log("V1.7.10 HMI GUI loaded. TX socket refresh test build.")
