...
- Decodes stopped/running/progress/current file/elapsed time/tool prompt/breakaway
...
class SendGui:
    ...
    def _build_ui(self) -> None:
        # Scrollable shell keeps lower controls reachable on smaller screens or
        ...
        outer = ttk.Frame(self.main_canvas, style="App.TFrame", padding=14)
        ...
        def _on_mousewheel(event) -> None:
            # Windows/macOS use event.delta; Linux can also emit Button-4/5.
            if event.num == 4:
        ...
        self._big_status_box(big_values, 1, "ELAPSED", self.elapsed_var)
        ...
        self.get_tools_btn = tk.Button(
            tools,
            text="Get Tools Data",
            command=self.get_tools_data,
            bg="#2b313c",
            fg=self.TEXT,
            activebackground="#3a4352",
            activeforeground=self.TEXT,
            relief="raised",
            bd=2,
            padx=12,
        ...
        self.generate_tools_btn = tk.Button(
            tools,
            text="Generate Text File",
            command=self.generate_tools_text_file,
            bg="#2b313c",
            fg=self.TEXT,
            activebackground="#3a4352",
            activeforeground=self.TEXT,
        ...
        # Belt-and-suspenders bindings in case command dispatch is swallowed by
        # the scrollable canvas or platform theme behavior.
        self.get_tools_btn.bind("<ButtonRelease-1>", lambda _event: self.get_tools_data())
        self.generate_tools_btn.bind("<ButtonRelease-1>", lambda _event: self.generate_tools_text_file())
        self.generate_tools_btn.bind("<Return>", lambda _event: self.generate_tools_text_file())
    ...
    def _load_brand_logo(self) -> None:
        """Load the FabLab Racing logo from the program directory if present."""
        candidates = [
    ...
    def _wire_target_preview_updates(self) -> None:
        self.remote_folder_var.trace_add("write", lambda *_: self._on_target_folder_changed())
        self._update_target_preview()

    def _on_target_folder_changed(self) -> None:
        """Called every time the target-folder field is edited.

        Pending/Failed queue items were previously stamped with whatever folder
        was in the field at the moment they were added, and never touched again.
        That meant changing the target after files were queued had no effect
        until the queue was cleared and refilled. Now the field is treated as
        the live target for anything not yet sent: Pending/Failed items are
        kept in sync with it. Sending/Done items are left alone so the queue
        still shows an accurate record of where each file actually went.
        """
        if self._sync_pending_queue_folders():
            return  # _refresh_queue_tree() already refreshed the preview
        self._update_target_preview()

    def _sync_pending_queue_folders(self) -> bool:
        if not getattr(self, "upload_queue", None):
            return False
        folder = normalize_masso_folder(self.remote_folder_var.get().strip() or "\\")
        changed = False
        for item in self.upload_queue:
            if item.get("status") in ("Pending", "Failed") and item["folder"] != folder:
                item["folder"] = folder
                changed = True
        if changed:
    ...
    def _auto_generate_tools_text_file_after_scan(self) -> None:
        """MASSO Link-style flow: after tools data is downloaded, save beside app and open."""
        if not self.tools_data:
            return
        self.log("Tools Data download complete; saving text file beside app")
        self.auto_save_and_open_tools_text_file()
    ...
    def auto_save_and_open_tools_text_file(self) -> None:
        """Save Tools Data beside the app/exe and open it, matching MASSO Link behavior."""
        self.log("Auto Generate Text File started")

        if self.tools_busy:
            self.log("Auto Generate Text File blocked: Tools Data is still loading")
            return

        if not self.tools_data:
            self.log("Auto Generate Text File blocked: no Tools Data loaded")
            return

        out_path = APP_DIR / self._default_tools_text_filename()
        self.log(f"Auto Generate Text File path: {out_path}")

        try:
            text_data = self._format_tools_text()
            out_path.write_text(text_data, encoding="utf-8")
            self.config["last_local_dir"] = str(out_path.parent)
            save_config(self.config)
            self.log(f"Generated Tools Data text file: {out_path} ({len(text_data)} characters)")
        except Exception as exc:
            self.log(f"Auto Generate Text File failed while writing: {exc}")
            messagebox.showerror(APP_NAME, f"Could not generate tools text file:\n\n{exc}")
            return

        opened, open_detail = open_file_with_default_app(out_path)
        self.log(f"Auto Generate Text File open detail: {open_detail}")

        if opened:
            self.log(f"Opened Tools Data text file: {out_path}")
        else:
            self.log(f"Tools Data text file saved, but auto-open failed: {open_detail}")
            messagebox.showwarning(
                APP_NAME,
                f"Tools Data text file was saved, but could not be opened automatically:\n\n"
                f"{out_path}\n\n"
                f"{open_detail}",
            )

    def generate_tools_text_file(self) -> None:
        self.log("Generate Text File clicked: opening manual save dialog")

        if self.tools_busy:
            self.log("Generate Text File blocked: Tools Data is still loading")
            messagebox.showinfo(APP_NAME, "Tools Data is still loading. Please wait for it to finish.")
            return

        if not self.tools_data:
            self.log("Generate Text File blocked: no Tools Data loaded")
            messagebox.showwarning(APP_NAME, "Click Get Tools Data first.")
            return

        default_name = self._default_tools_text_filename()
        self.log(f"Generate Text File: opening save dialog with default name {default_name}")
        ...
        if not output:
            self.log("Generate Text File cancelled: no file selected")
            return

        out_path = Path(output)
        self.log(f"Generate Text File selected path: {out_path}")
        ...
        try:
            ...
            self.log(f"Generate Text File failed while writing: {exc}")
            messagebox.showerror(APP_NAME, f"Could not generate tools text file:\n\n{exc}")
            return

        opened, open_detail = open_file_with_default_app(out_path)
        self.log(f"Generate Text File auto-open detail: {open_detail}")
        ...
        try:
            while True:
                ...
                if event_type == "log":
                    ...
                    self.generate_tools_btn.configure(state="normal")
                    self.log("Generate Text File button enabled")
                    if isinstance(payload, dict) and not payload.get("ok", False):
                        ...
                        self.root.after(250, self._auto_generate_tools_text_file_after_scan)
    ...
    def update_status(self, st: MassoStatus) -> None:
        ...
        self.elapsed_var.set(format_elapsed_seconds(st.elapsed_seconds))
