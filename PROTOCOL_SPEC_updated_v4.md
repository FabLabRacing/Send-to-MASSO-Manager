...
**Note**: This protocol specification is based on reverse engineering, packet captures, and controller testing. It includes confirmed SendToMasso V1.7.12 / V1.7.13 upload behavior. Many packet fields and status bytes are still not fully understood. Treat this as a working document, not official MASSO documentation.
...
- **Client Ports / Sockets**:
  ...
  - SendToMasso V1.7.10+ refreshes/recreates the TX socket before each upload. This gives each upload a fresh ephemeral source port and fixed a repeat-upload failure where the first upload after connect worked, but later uploads failed until reconnect.
