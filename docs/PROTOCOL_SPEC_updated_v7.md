...
**Note**: This protocol specification is based on reverse engineering, packet captures, and controller testing. It includes confirmed upload behavior from SendToMasso V1.7.12+ and the current Send-to-MASSO Manager V1.8.17 RC line. Many packet fields and status bytes are still not fully understood. Treat this as a working document, not official MASSO documentation.
...
When byte `7 != 0xFF`, the app should treat the machine as unsafe for upload.

Confirmed values from MASSO Link comparison captures:
...
- X/Y/Z/A/B motor alarm text in MASSO Link is shown as `<axis> Motor Alarm`.
- Air Pressure Low Alarm tripped byte `0x06`, but MASSO Link displayed `Machine Stopped` rather than a specific air-pressure text in the observed test. The status byte still distinguishes it.
- Unknown non-`0xFF` values should still be treated as faults and should block uploads.
