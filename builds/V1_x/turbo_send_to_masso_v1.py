#!/usr/bin/env python3
"""
TURBO Send-to-MASSO Manager - first run Tkinter proof of concept
...
APP_NAME = "TURBO Send-to-MASSO Manager"
...
class MassoClient:
    ...
    def start(self, host: str) -> bool:
        ...
        self.log(f"Bound UDP {self.local_port}; connecting to MASSO {self.host}:{CONTROLLER_PORT}")
    ...
    def _send_handshake(self) -> None:
        # Discovery packet from the original client/captures
        discovery_payload = bytes([0x03, 0x00, 0x02, 0xF8, 0x2A, 0x00, 0x00, 0x0B])
        ...
        try:
            self._send(with_crc(discovery_payload))
            time.sleep(0.15)
            self._send(with_crc(config_payload))
            self.log("Handshake sent")
        except Exception as exc:
            self.log(f"Handshake error: {exc}")

    def _keepalive_loop(self) -> None:
        # MASSO Link capture used: ec b9 03 00 01 10 00 00 00 03
        # Build it from payload instead of hardcoding the CRC.
        keepalive_payload = bytes([0x03, 0x00, 0x01, 0x10, 0x00, 0x00, 0x00, 0x03])
        keepalive = with_crc(keepalive_payload)
    ...
    def _build_start_upload_packet(self, filesize: int, remote_folder: str, filename: str) -> bytes:
        ...
        Observed capture layout, including checksum:
          ...
          [9:11]  00 00
          [11]    folder length including NUL
        ...
        payload.append(len(folder_b) + 1)  # capture used 0x11 for 16-byte folder + NUL
    ...
    def _build_data_packet(self, chunk_index: int, chunk: bytes) -> bytes:
        payload = bytearray()
        payload.extend(b"\x03\x00")
        payload.append(0x0B)
    ...
    def _upload_file_worker(self, local_path: str, remote_folder: str) -> None:
        ...
        try:
            ...
            for attempt in range(1, 4):
                ...
                code = start_ack[5:7]
                if code == b"\x00\x00":
                    self.log("Start upload accepted")
                    break
                self.log(f"Start upload rejected: code {code.hex(' ')}")
                start_ack = None
                time.sleep(0.4)
            ...
            with path.open("rb") as f:
                for chunk_index in range(total_chunks):
                    ...
                    if not chunk:
                        break
                    packet = self._build_data_packet(chunk_index, chunk)
...
class TurboSendGui:
    ...
    def _build_ui(self) -> None:
        ...
        self.log("First-run build loaded. Connect to MASSO before sending files.")
