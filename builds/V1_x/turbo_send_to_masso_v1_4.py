#!/usr/bin/env python3
"""
TURBO Send-to-MASSO Manager - first run Tkinter proof of concept (v1.4)
...
APP_NAME = "TURBO Send-to-MASSO Manager v1.4"
...
class MassoClient:
    ...
    def _build_data_packet(self, chunk_index: int, chunk: bytes, *, pad_to_full_chunk: bool = False) -> bytes:
        ...
        Official MASSO Link captures show:
          ...
          final chunk: actual remaining byte count, packet length 2+11+len+3

        V1.4 defaults back to that official behavior. The pad_to_full_chunk flag
        is left available for future testing, but the upload worker does not use it.
        """
        wire_chunk = chunk
        ...
        try:
            ...
            with path.open("rb") as f:
                for chunk_index in range(total_chunks):
                    ...
                    if not chunk:
                        break
                    # V1.4 uses the official MASSO Link final-chunk behavior:
                    # send the actual remaining byte count, not forced padding.
...
class TurboSendGui:
    ...
    def _build_ui(self) -> None:
        ...
        self.log("V1.4 build loaded. Connect to MASSO before sending files.")
