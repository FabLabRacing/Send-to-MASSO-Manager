#!/usr/bin/env python3
"""
Send-to-MASSO Manager - HMI-style Tkinter proof of concept (v1.7)
...
APP_NAME = "Send-to-MASSO Manager v1.7.13"
CONFIG_FILE = Path.home() / ".send_to_masso.json"
    ...
    def _upload_file_worker(self, local_path: str, remote_folder: str) -> None:
        ...
        try:
            ...
            if not self._reset_tx_socket_for_upload():
                self.log("Upload failed: could not refresh TX socket")
                return
...
class TurboSendGui:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(top, text="v1.7.13  •  Stable upload cleanup build", style="Version.TLabel").pack(side="right", pady=(9, 0))
        ...
        self._detail_row(status, 3, "Current / Last File", self.file_var)
        self._detail_row(status, 4, "Line", self.line_var)
        self._detail_row(status, 5, "Job Count", self.job_var)
        self._detail_row(status, 6, "Last Packet", self.last_seen_var)
        self._detail_row(status, 7, "Raw", self.raw_var)

        # File send panel
        send = self._panel(body, "Send File")
        send.grid(row=1, column=0, sticky="nsew", padx=(0, 8), pady=(0, 10))
        send.columnconfigure(1, weight=1)

        ttk.Label(send, text="Local file", style="Muted.TLabel").grid(row=0, column=0, sticky="w")
        self.local_file_var = tk.StringVar(value="")
        ttk.Entry(send, textvariable=self.local_file_var).grid(row=1, column=0, columnspan=2, sticky="ew", padx=(0, 8), pady=(2, 8))
        ttk.Button(send, text="Browse...", command=self.browse_file).grid(row=1, column=2, sticky="ew", pady=(2, 8))

        ttk.Label(send, text="Target MASSO folder", style="Muted.TLabel").grid(row=2, column=0, sticky="w")
        self.remote_folder_var = tk.StringVar(value=self.config.get("last_folder", "\\"))
        ttk.Entry(send, textvariable=self.remote_folder_var).grid(row=3, column=0, columnspan=2, sticky="ew", padx=(0, 8), pady=(2, 8))
        ttk.Label(send, text=r"example: \Test\  or  /Test/", style="Muted.TLabel").grid(row=3, column=2, sticky="w", pady=(2, 8))

        preview = ttk.Frame(send, style="SubPanel.TFrame", padding=8)
        preview.grid(row=4, column=0, columnspan=3, sticky="ew", pady=(4, 0))
        preview.columnconfigure(0, weight=1)
        ttk.Label(preview, text="MASSO TARGET PREVIEW", style="Muted.TLabel", background=self.PANEL_2).grid(row=0, column=0, sticky="w")
        self.target_preview_var = tk.StringVar(value="\\")
        self.target_validation_var = tk.StringVar(value="Target name looks OK")
        tk.Label(
            preview,
        ...
        self.send_btn.grid(row=5, column=0, sticky="ew", pady=(8, 0))
        self.upload_progress = ttk.Progressbar(send, maximum=100, mode="determinate")
        self.upload_progress.grid(row=5, column=1, columnspan=2, sticky="ew", padx=(8, 0), pady=(8, 0))

        self.last_sent_var = tk.StringVar(value="No successful upload yet")
        last = ttk.Frame(send, style="SubPanel.TFrame", padding=10)
        last.grid(row=6, column=0, columnspan=3, sticky="ew", pady=(12, 0))
        ...
        self.log("V1.7.13 HMI GUI loaded. Stable upload cleanup build.")
    ...
    def _detail_row(self, parent: tk.Widget, row: int, label: str, var: tk.StringVar) -> None:
        ...
        ttk.Label(box, text=label, style="Muted.TLabel").grid(row=0, column=0, sticky="w", padx=(0, 8))
        ttk.Label(box, textvariable=var, style="Value.TLabel", wraplength=330).grid(row=0, column=1, sticky="w")
    ...
    def _wire_target_preview_updates(self) -> None:
        self.local_file_var.trace_add("write", lambda *_: self._update_target_preview())
        self.remote_folder_var.trace_add("write", lambda *_: self._update_target_preview())
        self._update_target_preview()

    def _target_check(self) -> tuple[bool, list[str], list[str], str]:
        return validate_masso_name_parts(
            self.local_file_var.get().strip(),
            self.remote_folder_var.get().strip() or "\\",
        )

    def _update_target_preview(self) -> None:
        if not hasattr(self, "target_preview_var"):
            return
        ok, errors, warnings, preview = self._target_check()
    ...
    def send_selected_file(self) -> None:
        ...
        if not path:
            ...
            return
        target_ok, errors, warnings, preview = self._target_check()
        if not target_ok:
            messagebox.showwarning(APP_NAME, "Invalid MASSO target:\n\n" + "\n".join(errors))
            return
        if warnings:
    ...
    def update_status(self, st: MassoStatus) -> None:
        ...
        if st.connected and st.last_packet_time:
            age = max(0.0, time.monotonic() - st.last_packet_time)
            self.last_seen_var.set(f"{age:.1f}s ago")
        else:
    ...
    def _refresh_upload_button(self) -> None:
        ...
        if hasattr(self, "target_preview_var"):
            target_ok, _errors, _warnings, _preview = self._target_check()
        if allowed and file_ok and target_ok and not self.upload_busy:
